...
import { dialogSubmit } from '@/utils/util'
export default (Vue) => {
  function dialog (component, componentProps, modalProps) {
    const _vm = this
    modalProps = { ...modalProps, maskClosable: false } || { maskClosable: false }
    if (!_vm || !_vm._isVue) {
      return
    }
    ...
    const handle = function (checkFunction, afterHandel) {
      ...
      } else if (checkFunction instanceof Promise) {
        checkFunction.then(c => {
          c && afterHandel()
        })
      } else {
    ...
    const dialogInstance = new Vue({
      ...
      methods: {
        ...
        handleOk () {
          ...
          // 提交前验证
          const submitValidate = this.$refs._component.submitValidate
          if (submitValidate) {
            const res = submitValidate()
            res && handle(dialogSubmit(this.$refs._component), callback)
          } else handle(this.$refs._component.onOk || dialogSubmit(this.$refs._component), callback)
        }
      },
      render: function (h) {
        const that = this
