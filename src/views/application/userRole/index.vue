  ...
  <div class="userRole">
    <a-card :bordered="false">
      <a-form layout="inline" style="margin-bottom:20px;">
        ...
        <a-form-item>
          <a-button type="primary" @click="$refs.table.refresh(true)" v-if="$auth('userRole.list')">查询</a-button>
      ...
      <s-table
        ...
        <template #action="text">
          <span>
            <template>
              <a-button type="danger" @click="$globalHandlerDelete('/userRole/delete', { id: text }, $refs.table.refresh, 'userRoleListState')" v-if="$auth('userRole.delete')">删除</a-button>
            </template>
          </span>
        </template>
      </s-table>
...
const listState = ['orgListStateOptions', 'appListStateOptions', 'userListStateOptions', 'roleGroupListStateOptions', 'roleListStateOptions']
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        {
          ...
          customRender: (text, record) => {
            const userData = this.userListStateOptions.find(el => el.id === text)
            return userData && userData.account || ''
          }
        },
        {
          title: '姓名',
          align: 'center',
          customRender: (text, record) => {
            const userData = this.userListStateOptions.find(el => el.id === record.userId)
      ...
      loadData: parameter => {
        return getUserRoleList({ ...parameter, ...this.queryParam }).then(res => res)
      }
    }
  },
  ...
  created () {
    this.$bus.$on('updateUserRoleList', () => {
  ...
  methods: {
    statusChange (record) {
      ...
      this.$put('/userRole/modify', record, r => {
        if (r) {
          this.$refs.table.refresh()
