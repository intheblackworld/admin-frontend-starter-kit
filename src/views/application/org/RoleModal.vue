...
export default {
  ...
  methods: {
    ...
    initSelectsd () {
      this.selectedRows = this.orgRoleListStateOptions.filter(el => el.type === 'Role' && el.orgId === this.recordData.id && el.appId === this.appId)
      this.selectedRowKeys = this.selectedRows.map(el => el.roleId)
    },
  ...
  computed: {
    ...mapGetters(['sideCollapsed', 'microAppListStateOptions', 'orgListStateOptions', 'roleType', 'roleStatus', 'appListState', 'orgRoleListStateOptions']),
    barWidth () {
