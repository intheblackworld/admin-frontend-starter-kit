<template>
  <a-form :form="form">
    ...
    <a-form-item label="角色组名" :labelCol="labelCol" :wrapperCol="wrapperCol">
      <a-input v-decorator="['name', { rules: $validatorO.name('请输入角色组名') }]" placeholder="请输入角色组名"/>
    </a-form-item>
...
const fields = ['name', 'appId', 'status', 'summary']
const microFields = ['name', 'status', 'summary']
export default {
  props: {
    record: {
      type: Object,
  ...
  data () {
    return {
      ...
      requestData: {
        url: '/roleGroup',
        dispatchName: 'roleGroupListState'
      }
    }
  },
  computed: mapGetters(['roleGroupStatusOptions', 'appListStateFilterOptions', 'appListStateOptions', 'initAppId']),
  mounted () {
